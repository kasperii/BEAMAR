...
Shader "Custom/TransparentDistortion" {
Properties {
...
half4 frag (v2f i) : SV_Target
{
	...
	float2 offset = bump * _BumpAmt * _GrabTexture_TexelSize.xy;
		...
		fixed4 lighting = tex2D(_EmissionMap, _EmissionColor);
		half4 output = half4(albedo.rgb * lighting.rgb, albedo.a);
		half4 emission = tex2D(_EmissionMap, i.uvmain) * lighting;// *_EmissionColor;
		output.rgb += emission.rgb;
		//return output;
		//return output;


	//i.Emission = _Color.rgb;
