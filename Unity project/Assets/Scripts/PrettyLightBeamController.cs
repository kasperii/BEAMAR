      ...
      for(var i = 0; i < Line1.positionCount; i++) //Loop through every index and put x,y,z coordinates in array
      {
			...
			for (var u = 1; u < Line1.positionCount; u++)
			{
				if (LineIndex!=LineIndexOld)
				{
					...
					GameObject SoundSource = Instantiate(Cube2, LineIndex[u], Quaternion.Euler(90, 0, 0)) as GameObject;
					SoundSource.GetComponent<PrettyLightBeamSource>().InstantiateSoundSource(u, LineIndex);
